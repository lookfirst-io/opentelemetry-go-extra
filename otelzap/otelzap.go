...
type Logger struct {
	*zap.Logger
	...
	withTraceID     bool
	traceIDKey      string
	traceIDValueMap func(string) string
...
func New(logger *zap.Logger, opts ...Option) *Logger {
	l := &Logger{
		...
		traceIDValueMap: func(s string) string {
			return s
		},
	}
...
func (l *Logger) logFields(
	...
	if l.withTraceID {
		...
		fields = append(fields, zap.String(l.traceIDKey, l.traceValue(span)))
	}

	return fields
}

func (l *Logger) traceValue(span trace.Span) string {
	return l.traceIDValueMap(span.SpanContext().TraceID().String())
}
...
func (l *Logger) log(
	...
	if l.withTraceID {
		span := trace.SpanFromContext(ctx)
		kvs = append(kvs, log.String(l.traceIDKey, l.traceValue(span)))
	}
